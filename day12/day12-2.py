...
def is_start_loc_part2(char: str):
    return char == "S" or char == "a"
...
def main():
    ...
    for start_loc in start_locs:
        ...
        #for node in path_graph.walk_graph(start_node):
            ...
            #print("{node} -> [ {adj} ]".format( node = str(node), adj = ", ".join(str(adj_node) for adj_node in adj_nodes)))
        
        dist = path_graph.find_shortest_path_dist(start_node, end_node, hmap)

        if dist < min_dist:
            min_dist = dist
            print("shortest path dist = {dist}".format(dist = min_dist))
