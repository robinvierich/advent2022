...
def main():
    ...
    divisor_lcm = math.lcm(*(monkey.test.denom for monkey in monkies))

    for round in range(10000):
        print("round {r}".format(r = round + 1))
        for monkey in monkies:
            ...
            for item in monkey.items:
                # perform op (inspect)
                item.worry_level = int(monkey.operation.execute(item.worry_level))
                ...
                item.worry_level = item.worry_level % divisor_lcm
